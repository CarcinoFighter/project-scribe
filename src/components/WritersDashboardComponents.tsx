...
export function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span style={{ fontSize:10, fontWeight:700, letterSpacing:'0.05em', textTransform:'uppercase',
      padding:'2px 7px', borderRadius:'var(--r-pill)', background:s.bg, color:s.color,
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:8, height:100 }}>
        {bars.map((b,i) => (
          <div key={b.label} onMouseEnter={()=>setHov(i)} onMouseLeave={()=>setHov(null)}
            ...
            {hov===i && (
              <div className="anim-fade-up" style={{ position:'absolute', bottom:'calc(100% + 4px)', background:'var(--surface-2)', border:'1px solid var(--border-med)', borderRadius:'var(--r-md)', padding:'6px 10px', fontSize:11, color:'var(--text)', whiteSpace:'nowrap', boxShadow:'var(--sh-md)', pointerEvents:'none', zIndex:10, display:'flex', flexDirection:'column', gap:2 }}>
            ...
            <div
              ...
              style={{
                width:'100%', borderRadius:'var(--r-md) var(--r-md) var(--r-sm) var(--r-sm)',
    ...
    >
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:7 }}>
          <div style={{ width:24, height:24, borderRadius:'var(--r-md)', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:typeBg }}>
            <TypeIcon size={11} strokeWidth={2.2} style={{ color:typeColor }}/>
          </div>
          <StatusPill status={doc.status}/>
          {doc.isActive && <span style={{ fontSize:9, fontWeight:700, background:'var(--accent)', color:'#fff', borderRadius:'var(--r-xs)', padding:'1px 6px', letterSpacing:'0.05em', textTransform:'uppercase' }}>Live</span>}
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:3 }}>
          {doc.starred && <Star size={11} fill="var(--accent)" stroke="none"/>}
          <button className="tb-btn" onClick={e=>{e.stopPropagation();onContextMenu(e,doc.id);}} style={{ padding:'2px 4px', borderRadius:'var(--r-sm)', opacity:hov?1:0, transition:'opacity 0.10s' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginTop:2 }}>
        <div style={{ display:'flex', gap:3, flexWrap:'wrap', minWidth:0 }}>
          {(doc.tags||[]).slice(0,3).map((tag: string)=>(
            <span key={tag} style={{ fontSize:10, color:'var(--text-4)', background:'var(--bg-deep)', borderRadius:'var(--r-xs)', padding:'1px 6px' }}>#{tag}</span>
          ))}
        </div>
...
export function QuickAction({ icon: Icon, label, sublabel, color, bg, onClick }: { icon: React.ElementType; label:string; sublabel:string; color:string; bg:string; onClick:()=>void; }) {
  ...
  return (
    ...
    >
      <div style={{ width:32, height:32, borderRadius:'var(--r-md)', background:bg, border:`1px solid ${color.replace(')',',0.25)').replace('rgb','rgba')}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'transform 0.13s', transform:hov?'scale(1.06)':'scale(1)' }}>
...
// ── Skeleton loader ──────────────────────────────────────────────────────
export function Skeleton({ w='100%', h=14, r='var(--r-md)' }: { w?:string|number; h?:number|string; r?:number|string }) {
  return (
