...
// sortDocs and filterDocs kept as they are page-specific or used here
...
// Removed StatusPill (using shared component)
...
// Removed StatCard (using shared component)

// Removed ActivityChart (using shared component)

// ── Task card ─────────────────────────────────────────────────────────────────
// TaskCard moved to @/components/TaskCard

// Removed DocCard (using shared component)

// Removed QuickAction (using shared component)

// Removed EmptyDocState (using shared component)
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Check user department from profile
    if (user && !activeDeptKey) {
      if (user.department) {
        setActiveDeptKey(user.department);
      } else {
        setActiveDeptKey("Writers' Block");
      }
    }
  }, [user, activeDeptKey]);
      ...
      <div style={{ display:'flex', flex:1, minHeight:0 }}>
        ...
        <aside className="sidebar-col anim-slide-left dash-sidebar"
          ...
          {([
            ...
            if (!isFullSidebar && (item.id === 'articles' || item.id === 'blogs')) return null;
        ...
        <main className="dash-main" style={{ flex:1, padding:'28px 30px', overflowY:'auto', minWidth:0 }}>
          ...
          {activeNav==='home' && (
            <>
              {/* Greeting */}
              <div className="anim-fade-up" style={{ marginBottom: 32, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: '0 0 3px', letterSpacing: '-0.025em', lineHeight: 1.2 }}>
                    {getGreeting(user?.name)}
                  </h1>
                  <p style={{ fontSize: 13, color: 'var(--text-4)', margin: 0 }}>{getTodayLabel()}</p>
                </div>
              </div>
              ...
              {userLoading ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  <div style={{ display: 'flex', gap: 16 }}>
                    <div className="glass-raised" style={{ flex: 1, height: 100, borderRadius: 16, animation: 'lp-shimmer 1.5s infinite' }} />
                    <div className="glass-raised" style={{ flex: 1, height: 100, borderRadius: 16, animation: 'lp-shimmer 1.5s infinite 0.1s' }} />
                    <div className="glass-raised" style={{ flex: 1, height: 100, borderRadius: 16, animation: 'lp-shimmer 1.5s infinite 0.2s' }} />
                  </div>
                  <div className="glass-raised" style={{ height: 300, borderRadius: 20, animation: 'lp-shimmer 1.5s infinite 0.3s' }} />
                </div>
              ) : (
                <>
                  {(activeDeptKey === 'Development' || (activeDeptKey === 'Leadership' && selectedDept === 'Development')) && (
                    <DevelopmentDashboard />
                  )}

                  {(activeDeptKey === 'Marketing' || (activeDeptKey === 'Leadership' && selectedDept === 'Marketing')) && (
                    <MarketingDashboard />
                  )}

                  {(activeDeptKey === 'Design Lab' || (activeDeptKey === 'Leadership' && selectedDept === 'Design Lab')) && (
                    <DesignLabDashboard />
                  )}
