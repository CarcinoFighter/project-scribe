...
function ReviewQueue({ 
  ...
  if (docs.length === 0) return (
    <div className="mb-12 anim-fade-in p-8 text-center border border-dashed border-[var(--border-med)] bg-[var(--bg-deep)]">
      ...
      <p className="text-sm font-medium text-[var(--text-4)] italic">No documents currently awaiting review.</p>
    </div>
  );
    ...
    <div className="mb-12 anim-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[var(--text)]">Review Queue</h2>
            <p className="text-xs text-[var(--text-4)] uppercase tracking-widest font-bold">Administrator approval required</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {docs.map((doc) => {
          ...
          return (
            <div key={doc.id} className="db-card p-5 border border-[var(--border-med)] flex flex-col gap-4 group">
              <div className="flex items-start justify-between gap-3">
                <div className={`w-9 h-9 flex items-center justify-center flex-shrink-0 ${bg}`}>
                  <Icon size={18} className={color} />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold text-[var(--text)] mb-0.5 truncate group-hover:text-[var(--accent)] cursor-pointer" onClick={() => {
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-[var(--text-4)] font-bold uppercase tracking-tight">{doc.type.replace('_', ' ')}</span>
                    <span className="text-[10px] text-[var(--text-4)] opacity-50">•</span>
                    <span className="text-[10px] text-[var(--text-4)] font-medium">{new Date(doc.updated_at).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5 px-3 py-2 bg-[var(--bg-deep)] border border-[var(--border-med)]">
                {doc.author?.avatar_url ? (
                  <img src={doc.author.avatar_url} alt={doc.author.name || ''} className="w-[20px] h-[20px] rounded-full object-cover" />
                ) : (
                ...
                <span className="text-[10px] text-[var(--text-3)] font-black uppercase tracking-tighter">By {doc.author?.name || 'Unknown'}</span>
              </div>
...
function ProofreaderQueue({ 
  ...
  if (docs.length === 0) return (
    <div className="mb-12 anim-fade-in p-8 text-center border border-dashed border-[var(--border-med)] bg-[var(--bg-deep)]">
      ...
      <p className="text-sm font-medium text-[var(--text-4)] italic">No articles currently assigned for proofreading.</p>
    </div>
  );
    ...
    <div className="mb-12 anim-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[var(--text)]">Proofreading Queue</h2>
            <p className="text-xs text-[var(--text-4)] uppercase tracking-widest font-bold">Assigned editorial reviews</p>
          </div>
        </div>
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-[var(--text-4)] font-bold uppercase tracking-tight">{doc.type.replace('_', ' ')}</span>
                    <span className="text-[10px] text-[var(--text-4)] opacity-50">•</span>
                    <span className="text-[10px] text-[var(--text-4)] font-medium">By {doc.author?.name || 'Unknown'}</span>
                  </div>
                </div>
              </div>
...
function AssignmentQueue({ 
  ...
  if (docs.length === 0) return (
    <div className="mb-12 anim-fade-in p-8 text-center border border-dashed border-[var(--border-med)] bg-[var(--bg-deep)]">
      ...
      <p className="text-sm font-medium text-[var(--text-4)] italic">No articles currently awaiting proofreader assignment.</p>
    </div>
  );
    ...
    <div className="mb-12 anim-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[var(--text)]">Assignment Queue</h2>
            <p className="text-xs text-[var(--text-4)] uppercase tracking-widest font-bold">Awaiting proofreader dispatch</p>
          </div>
        </div>
          ...
          return (
            <div key={doc.id} className="db-card p-5 border border-[var(--border-med)] flex flex-col gap-4 group">
              ...
              <div className="space-y-2.5 mt-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-4)] opacity-60">Dispatch Proofreader</p>
                <MultiPersonSelect 
                  selectedIds={[]}
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-10 anim-fade-in">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b-2 border-[var(--ink)]">
                <div>
                  <p className="db-page-sub mb-1">Administrative · Control Center</p>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-[var(--accent-sub)] border border-[var(--rule)]">
                    <span className="db-cap text-[var(--accent)] font-black">{reviewDocs.length + proofreaderDocs.length}</span>
            ...
            {loading ? (
              <div className="py-24 flex flex-col items-center justify-center gap-5 text-[var(--text-4)]">
                ...
                <span className="db-cap font-bold tracking-widest">Hydrating Queues...</span>
              </div>
            ) : (
